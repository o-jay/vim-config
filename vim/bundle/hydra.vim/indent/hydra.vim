...
setlocal indentexpr=GetHydraIndent(v:lnum)
setlocal indentkeys=!^F,o,O,<:>,0),0],0},=elif

" Only define the function once.
if exists("*GetHydraIndent")
  finish
endif
...
function! GetHydraIndent(lnum)
  ...
  if cline =~ '^\s*\(if\|when\|for\|while\|switch\|case\|try\)\>'
  ...
  if cline =~ '^\s*\(elif\|else\)\>'
    return s:FindStartLine(a:lnum, '^\s*\(if\|when\|elif\|case\)')
  endif

  if pline =~ ':\s*$'
    "return s:FindStartLine(plnum, '(^\s*\(if\|when\|else\|elif\|case\|of\|try\|except\|finally\)\>)\|\<do\>') + &sw
    return s:FindStartLine(plnum, '^\s*\(if\|when\|else\|elif\|for\|while\|switch\|case\|try\|except\|finally\)\>') + &sw
  endif

  if pline =~ '=\s*$'
    return s:FindStartLine(plnum, '^\s*\(fn\|template\|macro\|iterator\)\>') + &sw
  endif

  " if we got here, this should be the begging of a multi-line if expression for example
  if pline =~ '^\s*\(if\|when\|fn\|iterator\|macro\|template\|for\|while\)[^:]*$'
    return plindent + &sw
  endif

  if pline =~ '\(type\|import\|module\|let\)\s*$'
    \ || pline =~ '=\s*\(struct\|enum\|generic\)'
    return plindent + &sw
  endif

  " If the previous line was a stop-execution statement...
  if pline =~ '^\s*\(break\|continue\|throw\|return\)\>'
